...
parser.add_argument('--run_mode', type=str, default='train', choices=['train','test'], help='run mode')
...
# evaluate
def model_eval_o(model, dataset, voc_size=[-1,-1,153], epoch=0):
    # evaluate
    print('')
    smm_record = []
    ...
    for step in range(data_size):
        ...
        y_pred_prob = []
        y_pred_label = []
        adms = prepare_mimic_sample_dual_persist(dataset, config["output_size"], index=step)
        for adm_idx, adm in enumerate(adms):
            input_vec1, input_vec2, output_vec, o = adm
            if len(output_vec) == 0 and len(output_vec[0]) == 0:
                continue
            if config["has_history"]:
            ...
            target_output1 = np.zeros(voc_size[2])
            for output_idx in range(len(out[0])):
                target_output1[2+output_idx] = out[0][output_idx]

            y_gt_tmp = np.zeros(voc_size[2])
            y_gt_tmp[dataset[step][adm_idx][2]] = 1
        ...
        llprint('\rEval--Epoch: %d, Step: %d/%d' % (epoch, step, len(dataset)))
